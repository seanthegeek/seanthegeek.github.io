...
In their criminal referral, an online-communication platform, "Company H" provided logs of chats between Conspirator A and Stokes on Company H's infrastructure after their support system was compromised, described on paragraph 65. Coconspirator A was identified because the same IP address was used for an account used by Coconspirator A and another account used to communicate with Stokes, who used an existing account under the name "Bouquet". The Bouquet account contained a post with an image of homework bearing the name "Peter William Stokes". Coconspirator A confirmed to the FBI that Stokes used the moniker Bouquet.
